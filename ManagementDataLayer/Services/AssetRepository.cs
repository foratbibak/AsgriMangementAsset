...
namespace ManagementDataLayer.Services
{
    ...
    {
        //private string connectionString = @"Provider=Microsoft.Jet.OLEDB.4.0;Data Source=C:\Users\forat\source\repos\AsgriMangementAsset\ManagementDataLayer\Context\SchoolManagement1.accdb;";
        private string connectionString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\Users\forat\source\repos\AsgriMangementAsset\ManagementDataLayer\Context\SchoolManagement1.accdb;";
        ...
        public bool Insert(string AssetCode, string Status, string AssetTitle, string AssetCompany, string AssetCountry, decimal AssetPrice, string AssetColor, string AssetModel, string AssetSize)
    {
        ...
        try
        {
            ...
            OleDbCommand command = new OleDbCommand(query, connection);
                ...
                command.Parameters.AddWithValue("@AssetPrice", AssetPrice == 0 ? "نامشخص" : AssetPrice.ToString("N0"));
        ...
        public bool Update(int AssetLabelNumber,string AssetCode, string Status, string AssetTitle, string AssetCompany, string AssetCountry, decimal AssetPrice, string AssetColor, string AssetModel, string AssetSize)
        {
            OleDbConnection connection=new OleDbConnection(connectionString);
            try
            {
                ...
                command.Parameters.AddWithValue("@AssetCompany", string.IsNullOrWhiteSpace(AssetCompany) ? "نامشخص" : AssetCompany);
                command.Parameters.AddWithValue("@AssetCountry", string.IsNullOrWhiteSpace(AssetCountry) ? "نامشخص" : AssetCountry);
                command.Parameters.AddWithValue("@AssetPrice", AssetPrice == 0 ? 0 : AssetPrice);
                command.Parameters.AddWithValue("@AssetColor", string.IsNullOrWhiteSpace(AssetColor) ? "نامشخص" : AssetColor);
                command.Parameters.AddWithValue("@AssetModel", string.IsNullOrWhiteSpace(AssetModel) ? "نامشخص" : AssetModel);
                command.Parameters.AddWithValue("@AssetSize", string.IsNullOrWhiteSpace(AssetSize) ? "نامشخص" : AssetSize);
